...
export default function PrivatePayrollHistoryPage() {
  ...
  const fetchRuns = useCallback(async () => {
    ...
    try {
      ...
      const response = await walletAuthenticatedFetch({
        wallet,
        signMessage,
        path: `/api/v1/history?wallet=${wallet}`,
        method: "GET",
      });
  ...
  const exportRunsJson = () => {
    downloadJson(
      `auraflow_private_payroll_runs_${new Date().toISOString().split("T")[0]}.json`,
      filteredRuns,
    );
  };
  ...
  const exportRunsCsv = () => {
    downloadCsv(
      `auraflow_private_payroll_runs_${new Date().toISOString().split("T")[0]}.csv`,
      [
        [
          "date",
          "pay_period",
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            downloadJson(
                              `auraflow_private_payroll_run_${run.id}.json`,
                              run,
                            )
                          }
