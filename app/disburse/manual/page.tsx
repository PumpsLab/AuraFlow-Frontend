...
function ManualBatchPayrollContent() {
      ...
      try {
        ...
        const [employeesResponse, companyResponse] = await Promise.all([
          walletAuthenticatedFetch({
            ...
            signMessage,
            path: `/api/v1/employees?employerWallet=${employerWallet}`,
            method: "GET",
          }),
          walletAuthenticatedFetch({
            wallet: employerWallet,
            signMessage,
            path: `/api/v1/companies/me?employerWallet=${employerWallet}`,
            method: "GET",
          }),
        ]);
  ...
  const refreshPrivateBalance = useCallback(async () => {
    ...
    try {
      ...
      const response = await walletAuthenticatedFetch({
        wallet,
        signMessage,
        path: `/api/v1/companies/${companyId}/balance?wallet=${wallet}`,
        method: "GET",
      });
      ...
      const raw = typeof (payload as any).balance === "string" ? (payload as any).balance : ((payload as any).balance?.usdc ?? "0");
      const normalized = parseFloat(String(raw)) || 0;
  ...
  const runPayroll = useCallback(async () => {
    ...
    try {
      // Step 1: build XDR on backend
      updateStep(0, { status: "active", label: "Building transaction on backend…" });
      const buildRes = await walletAuthenticatedFetch({
        wallet: publicKey,
        signMessage,
        path: "/api/v1/private-payroll/build-transaction",
        method: "POST",
        body: {
      ...
      const buildPayload = (await buildRes.json()) as {
        ok?: boolean; error?: string; xdr?: string; totalAmountMicro?: number; missingAmountMicro?: number;
      };
      if (!buildRes.ok) {
        if (buildRes.status === 409 && typeof buildPayload.missingAmountMicro === "number") {
          const missingUsdc = buildPayload.missingAmountMicro / 1_000_000;
          setSteps([{ label: `Treasury is short by ${missingUsdc.toFixed(2)} USDC`, status: "error" }]);
        } else { updateStep(0, { status: "error" }); }
        throw new Error(buildPayload.error || "Build transaction failed");
      }
      if (!buildPayload.xdr) throw new Error("No XDR returned");

      updateStep(0, { status: "done", label: "Transaction built — sign in Freighter…" });

      // Step 2: sign with Freighter
      const signedXdr = await signTransaction!(buildPayload.xdr);
      validEmployees.forEach((_, idx) => updateStep(idx + 1, { status: "active" }));

      // Step 3: submit — include wallet so WalletAuthGuard can resolve expectedWallet
      const submitRes = await walletAuthenticatedFetch({
        wallet: publicKey,
        signMessage,
        path: `/api/v1/private-payroll/submit?wallet=${publicKey}`,
        method: "POST",
        body: { signedXdr, wallet: publicKey, employerWallet: publicKey },
      });
      const submitPayload = (await submitRes.json()) as { ok?: boolean; error?: string; txHash?: string; hash?: string; status?: string };
      if (!submitRes.ok) {
        // surface Horizon/Soroban extras if present
        const detail = (submitPayload as any)?.extras?.result_codes || (submitPayload as any)?.detail || "";
        updateStep(0, { status: "error", label: `Submit failed: ${submitPayload.error || detail || submitRes.status}` });
        throw new Error(submitPayload.error || `Submit failed: ${submitRes.status} ${detail}`);
      }
      const txHash = submitPayload.txHash || submitPayload.hash || "";
      if (!txHash) throw new Error("No txHash returned");

      // Poll Soroban RPC for confirmation (pending → success)
      try {
        const { HORIZON_URL } = await import("@/lib/wallet/contract-config");
        // Use Horizon for tx status if available, else just trust hash
        console.log("[private-payroll] submitted hash", txHash);
      } catch {}

      updateStep(0, { status: "done", label: "Company treasury balance confirmed" });
      validEmployees.forEach((_, idx) => updateStep(idx + 1, { status: "done", sig: txHash }));
                  ...
                  <div className="space-y-4">
                    {steps.map((step, index) => (
                      <div key={index} className="group flex items-center gap-4">
                        ...
                        {step.sig && (
                          <a
                            href={`https://stellarchain.io/transactions/${step.sig}`}
                            target="_blank"
          ...
          >
            ...
            {successModal.transferSig && (
              <div className="mb-8 space-y-2">
                <a
                  href={`https://stellarchain.io/transactions/${successModal.transferSig}`}
                  target="_blank"
                  rel="noopener noreferrer"
