...
export function CompliancePageContent() {
    ...
    try {
      const [tokenRes, eventsRes, statementsRes] = await Promise.all([
        walletAuthenticatedFetch({
          path: `/api/v1/auditor-tokens?employerWallet=${walletAddr}`,
          method: "GET",
          wallet: walletAddr,
          signMessage,
        }),
        walletAuthenticatedFetch({
          path: `/api/v1/compliance/events?wallet=${walletAddr}&limit=25`,
          method: "GET",
          wallet: walletAddr,
          signMessage,
        }),
        walletAuthenticatedFetch({
          path: `/api/v1/payroll-runs/statements?scope=employer&employerWallet=${walletAddr}`,
          method: "GET",
          wallet: walletAddr,
          signMessage,
        }),
    ...
    async (scope: "owner" | "auditor" | "employee") => {
      ...
      try {
        const response = await walletAuthenticatedFetch({
          path: `/api/v1/compliance/export?wallet=${walletAddr}&scope=${scope}`,
          method: "GET",
          wallet: walletAddr,
          signMessage,
        });
        ...
        const filename = `auraflow-compliance-${scope}-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const handleDownloadStatementsCsv = useCallback(() => {
    ...
    downloadTextFile(
      `auraflow-payroll-statements-${new Date().toISOString().slice(0, 10)}.csv`,
      csv,
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[#a8a8aa] sm:text-base">
                Stellar and TEE enclaves secure private execution. AuraFlow controls disclosure, statements, and auditor review.
              </p>
            </div>
          </div>
        ...
        <div className="grid items-start gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          <section className="rounded-3xl border border-white/5 bg-[#0a0a0a] p-6">
            ...
            <div className="mt-4 flex items-center gap-2 text-xs text-[#7f7f84]">
              <InfoHint label="TEE enclaves handle screened private execution. AuraFlow handles auditor access, statements, exports, and activity records." />
