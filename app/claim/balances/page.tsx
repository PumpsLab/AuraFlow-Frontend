...
export default function ClaimBalancesPage() {
  ...
  const handleInitialize = async () => {
    ...
    try {
      const currentStatus = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://auraflow-backend-ulnl.onrender.com"}/api/v1/employees/private-init?employeeWallet=${publicKey.toBase58()}`);
          ...
          <div>
            ...
            <p className="mt-2 max-w-lg text-sm leading-relaxed text-[#a8a8aa]">
              {hasPrivatePayrollMode
                ? "AuraFlow shows your private balance and private payroll payouts. Stellar is where you withdraw."
                : "Stellar shows wallet funds. AuraFlow shows your private balance and live payroll accrual."}
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl border border-[#a855f7]/30 bg-[#a855f7]/10 p-8">
              ...
              <p className="text-xs italic leading-relaxed text-[#9ce8d5]">
                Your balances are encrypted and only accessible by your private key. AuraFlow never stores your unencrypted balance or transaction history.
              </p>
            </div>
          </div>
        </div>
