...
import { getOrCreateCachedTeeToken, loadCachedTeeToken } from "@/lib/tee-auth-cache";
      ...
      try {
        const [employeeRes, streamRes, statementRes] = await Promise.all([
          ...
          walletAuthenticatedFetch({
            wallet: walletAddr,
            signMessage,
            path: `/api/v1/payroll-runs/statements?scope=employer&employerWallet=${walletAddr}&employeeId=${employeeId}`,
          }),
        ]);
        ...
        if (!tokenCache.current) {
          tokenCache.current =
            loadCachedTeeToken(publicKey) ??
            (await getOrCreateCachedTeeToken(publicKey, async () =>
              fetchTeeAuthToken(publicKey, signMessage),
            ));
        }

        const summaryRes = await fetch(
          `/api/v1/payroll/employee?employeeWallet=${nextEmployee.wallet}`,
          {
            headers: {
