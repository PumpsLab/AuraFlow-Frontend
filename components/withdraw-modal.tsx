...
export function WithdrawModal({
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      toast.info("Withdrawing from treasury…");
      const res = await walletAuthenticatedFetch({
        path: `/api/v1/companies/${companyId}/withdraw?wallet=${publicKey}`,
        method: "POST",
        signMessage: signMessage!,
        wallet: publicKey!,
        body: { amount: val, destinationAddress: publicKey },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Withdraw failed");
      const txHash = data.txHash as string;
      toast.success("Withdrawal submitted to Stellar!");
      // Save to history so Treasury Volume / Transactions update
      try {
        await walletAuthenticatedFetch({
          ...
          signMessage: signMessage!,
          wallet: publicKey!,
          body: { wallet: publicKey, kind: "setup-action", data: { type: "treasury-withdraw", amount: val, txSig: txHash, status: "success" } },
        });
      } catch (e) { console.warn("history withdraw save failed", e); }
        ...
        <div className="p-6 overflow-y-auto overflow-x-hidden">
          {successSig ? (
            <div className="flex flex-col items-center justify-center py-6 animate-in fade-in zoom-in duration-300">
              ...
              <Link
                href={`https://stellarchain.io/transactions/${successSig}`}
                target="_blank"
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-2 text-xs font-medium text-[#8f8f95] px-1">
                ...
                Stellar Testnet Network Online
              </div>
