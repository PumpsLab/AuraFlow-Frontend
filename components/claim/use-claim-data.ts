...
const CLAIM_DATA_CACHE_KEY = "auraflow:claim-data-cache";
...
export function useClaimData() {
  ...
  const fetchPrivateInitStatus = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      try {
        const response = await fetch(`/api/v1/employees/private-init?employeeWallet=${publicKey}`);
    ...
    async (options?: { silent?: boolean; force?: boolean; interactive?: boolean }) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/v1/payroll/employee?employeeWallet=${publicKey}`,
          {
